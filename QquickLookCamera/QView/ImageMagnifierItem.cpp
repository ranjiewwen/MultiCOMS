...
int ImageMagnifierItem::saveToFile() const
{
	...
	if (cmosNumber == 0)
	{
		...
		m_file->save(bufferConverted, m_range.height(), m_range.width(), /*"../QquickLookCamera/BMP/"*/"C:\\Users\\CMOS\\Desktop\\MultiCOMS\\BMP\\", name);  //C:\Users\ranji\Desktop\MultiCMOS\QquickLookCamera\QView\ImageMagnifierItem.cpp
	}
	else  //灰度图像直接存储原始bayer(8为深度)数据   //如果全部转换后存储，8->32转换有错。
		m_file->save(buffer, m_range.height(), m_range.width(), "C:\\Users\\ranji\\Desktop\\MultiCMOS\\QquickLookCamera\\", name);  //不能为空指针，否则访问地址出错，而且放在displayModeDisplay()会实时保存
